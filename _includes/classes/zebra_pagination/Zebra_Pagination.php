...
class Zebra_Pagination
{
    ...
    private $_properties = array(
		...
		// this is used for when method is ajax, this is the function to call when changing pages
		'ajax_function'				=>	'change_page',
    );
    ...
    public function render($return_output = false)
    {
        ...
        $output = '<div class="Zebra_Pagination pages"><ol>';
    ...
    private function _build_uri($page)
    {
        ...
        // if page propagation is to be done through GET
        } else if ($this->_properties['method'] == 'get') {
        ...
        } else if ($this->_properties['method'] == 'ajax') {
            ...
            // make sure the returned HTML is W3C compliant
			return 'javascript:'.$this->_properties['ajax_function'].'(\''.$query.'\');';
        }
    ...
    private function _show_next()
    {
        ...
        if ($this->_properties['always_show_navigation'] || $this->_properties['total_pages'] > $this->_properties['selectable_pages'])
            ...
            $output = '<li><a href="' .
                ...
                '" class="button next i-next navigation ' . ($this->_properties['reverse'] ? 'previous' : 'next') . ($this->_properties['page'] == $this->_properties['total_pages'] ? ' disabled' : '') . '"' .
    ...
    private function _show_pages()
    {
        ...
        if ($this->_properties['total_pages'] <= $this->_properties['selectable_pages']) {
            ...
            for (
                ...
                $output .= '<li '.($this->_properties['page'] == $i ? 'class="current"' : '') .'><a href="' . $this->_build_uri($i) . '" ' .

                    // make sure to highlight the currently selected page
                    ($this->_properties['page'] == $i ? 'class="button button-dark"' : 'class="button"') . '>' .
    ...
    private function _show_previous()
    {
        ...
        if ($this->_properties['always_show_navigation'] || $this->_properties['total_pages'] > $this->_properties['selectable_pages'])
            ...
            $output = '<li><a href="' .
                ...
                // also different class if links are in reverse order
                '" class="button prev i-prev navigation ' . ($this->_properties['reverse'] ? 'next' : 'previous') . ($this->_properties['page'] == 1 ? ' disabled' : '') . '"' .
	...
	public function set_ajax_function($name)
	{
		$this->_properties['ajax_function'] = $name;	
	}
