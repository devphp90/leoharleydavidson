...
class Products {
	...
	public function create_tmp()
	{
		...
		if (sizeof($filters)) {
			...
			if (isset($filters['brand']) && $filters['brand'] != '') $where[] = 'product.brand = "'.$this->mysqli->escape_string($filters['brand']).'"';
			if (isset($filters['featured_products']) && $filters['featured_products'] == 1) $where[] = 'product.featured = 1';
			if (isset($filters['new_products']) && $filters['new_products'] == 1) $where[] = '(product.used = 0 AND product.date_created >= DATE_SUB("'.$current_datetime.'", INTERVAL "'.$config_site['cf_new_products_no_days'].'" DAY))';
			if (isset($filters['top_sellers']) && $filters['top_sellers'] == 1) $where[] = 'has_been_sold(product.id,0,IF(product_image_variant.variant_code IS NOT NULL,product_image_variant.variant_code,"")) > 0';
			if (isset($filters['combo_deals']) && $filters['combo_deals'] == 1) $where[] = 'product.product_type = 1';
			if (isset($filters['bundled_products']) && $filters['bundled_products'] == 1) $where[] = 'product.product_type = 2';
		...
		if (sizeof($filters)) {		
			...
			if (isset($filters['on_sale']) && $filters['on_sale'] == 1) $where_price[] = '(t.sell_price < t.price)';
		}		
		...
		if (!$this->mysqli->query($sql)) throw new Exception('An error occured while trying to create temporary table.'."\r\n\r\n".$this->mysqli->error);	
	...
	public function count_products($filters=array())
	{
		...
		*/		
		...
		if (!$result = $this->mysqli->query($sql)) throw new Exception('An error occured while trying to query table.'."\r\n\r\n".$this->mysqli->error);		
	...
	public function get_products($filters=array(), $orderby=0, $offset=0, $limit=12, $create_tmp=0)
	{
		...
		if ($id_customer_type) {
			if (!$result = $this->mysqli->query('SELECT * FROM customer_type WHERE id = "'.$id_customer_type.'" LIMIT 1')) throw new Exception('An error occured while trying to get customer type.');
		...
		*/		
		...
		if ($result->num_rows) {	
			while ($row = $result->fetch_assoc()) {
				...
				if (!empty($row['variant_code'])) $url .= '?variant_code='.$row['variant_code'];
				...
				// totals
				++$this->total_products;
				if ($row['featured']) ++$this->total_featured;
				if ($row['new_product']) ++$this->total_new;
				if ($on_sale) ++$this->total_on_sale;
				if ($row['used']) ++$this->total_used;
				if ($row['product_type'] == 1) ++$this->total_combo;
				if ($row['product_type'] == 2) ++$this->total_bundled;
				
				if (!empty($row['brand'])) {
					if (!isset($this->brands[$row['brand']])) $this->brands[$row['brand']] = array('brand'=>$row['brand'],'total'=>1);
					else ++$this->brands[$row['brand']]['total'];
				}
				
				if ($row['avg_rating'] > 0) {
					if (!isset($this->ratings[$row['avg_rating']])) $this->ratings[$row['avg_rating']] = array('avg_rating'=>$row['avg_rating'],'total'=>1);
					else ++$this->ratings[$row['avg_rating']]['total'];
				}
				
				$this->ids[] = $row['id'];
				
				if ($row['sell_price'] > $this->max_price) $this->max_price = $row['sell_price'];
			}
		}
		...
		foreach ($products as $key => $row) {
			...
			if (!$stmt_has_been_sold->bind_param("iis", $row['id'], $row['id_product_variant'], $row['variant_code'])) throw new Exception('An error occured while trying to bind params to check if product has been sold statement.'."\r\n\r\n".$this->mysqli->error);
			
			/* Execute the statement */
			if (!$stmt_has_been_sold->execute()) throw new Exception('An error occured while trying to execute check if product has been sold statement.'."\r\n\r\n".$this->mysqli->error);				
			...
			if (!empty($row['variant_code'])) {
				...
				*/
				...
				foreach ($variant_codes as $row_variant_code) {
					...
					if (!$stmt_variant_image->bind_param("is", $row['id'], $row_variant_code)) throw new Exception('An error occured while trying to bind params to get variant cover image statement.'."\r\n\r\n".$this->mysqli->error);
			...
			if (!$filename) {
				if (!$stmt_image->bind_param("i", $row['id'])) throw new Exception('An error occured while trying to bind params to get image statement.'."\r\n\r\n".$this->mysqli->error);
			...
			if ($temp_filename && is_file(dirname(__FILE__).'/../../images/products/thumb/'.$temp_filename)) $filename_thumb = '/images/products/thumb/'.$temp_filename;
			else $filename_thumb = '';
			
			if ($temp_filename && is_file(dirname(__FILE__).'/../../images/products/cover/'.$temp_filename)) $filename_cover = '/images/products/cover/'.$temp_filename;
			else $filename_cover = '';
			
			if ($temp_filename && is_file(dirname(__FILE__).'/../../images/products/suggest/'.$temp_filename)) $filename_suggest = '/images/products/suggest/'.$temp_filename;
			else $filename_suggest = '';
			
			if ($temp_filename && is_file(dirname(__FILE__).'/../../images/products/zoom/'.$temp_filename)) $filename_zoom = '/images/products/zoom/'.$temp_filename;
			else $filename_zoom = '';
			...
			if (!$row['has_variants']) {		
				...
				if (!$stmt_in_stock->bind_param("iii", $row['id'], $id_product_variant, $qty)) throw new Exception('An error occured while trying to bind params to check if product is in stock statement.'."\r\n\r\n".$this->mysqli->error);
			...
			// check if at least one variant is in stock
				...
				if (!empty($row['variant_code'])) {					
					if (!$stmt_variants_in_stock->bind_param("si", $row['variant_code'], $row['id'])) throw new Exception('An error occured while trying to bind params to check if product variants is in stock statement.'."\r\n\r\n".$this->mysqli->error);
					...
					if (!$stmt_variants_in_stock_no_variant_code->bind_param("i", $row['id'])) throw new Exception('An error occured while trying to bind params to check if product variants is in stock statement.'."\r\n\r\n".$this->mysqli->error);
				...
				if (!$stmt_single_variant->bind_param("si", $row['variant_code'], $row['id'])) throw new Exception('An error occured while trying to bind params to check if product is single variant statement.'."\r\n\r\n".$this->mysqli->error);
	...
	public function get_filter_by_featured_products()
	{
		...
		if ($config_site['cf_show_featured_products_menu']) { 		
			...
			if (isset($array['featured_products']) && $array['featured_products'] == 1) unset($array['featured_products']);
			else $array['featured_products'] = 1;
	...
	public function get_filter_by_products_on_sale()
	{
		...
		if ($config_site['cf_show_on_sale_menu']) {
			...
			if (isset($array['on_sale']) && $array['on_sale'] == 1) unset($array['on_sale']);
			else $array['on_sale'] = 1;
	...
	public function get_filter_by_new_products()
	{
		...
		if ($config_site['cf_show_new_products_menu']) {		
			...
			if (isset($array['new_products']) && $array['new_products'] == 1) unset($array['new_products']);
			else $array['new_products'] = 1;
	...
	public function get_filter_by_top_sellers()
	{
		...
		if ($config_site['cf_show_top_sellers_menu']) {
			...
			if (isset($array['top_sellers']) && $array['top_sellers'] == 1) unset($array['top_sellers']);
			else $array['top_sellers'] = 1;
	...
	public function get_filter_by_combo_deals()
	{
		...
		if ($config_site['cf_show_combo_deals_menu']) {
			...
			if (isset($array['combo_deals']) && $array['combo_deals'] == 1) unset($array['combo_deals']);
			else $array['combo_deals'] = 1;
	...
	public function get_filter_by_bundled_products()
	{
		...
		if ($config_site['cf_show_bundled_product_menu']) {		
			...
			if (isset($array['bundled_products']) && $array['bundled_products'] == 1) unset($array['bundled_products']);
			else $array['bundled_products'] = 1;
	...
	public function get_filter_by_brand()
	{
		...
		if ($config_site['cf_show_brands']) {		
			...
			/*
			$sql = str_replace('{{column_names}}','product.brand, COUNT(product.id) AS total',$this->sql);
			$sql .= ' WHERE product.brand != "" GROUP BY product.brand ORDER BY brand ASC';
	...
	public function get_filter_by_price()
	{
		...
		if ($config_site['display_price'] && $config_site['cf_show_price_range']) {				
			...
			if (!$result = $this->mysqli->query('SELECT price_increment FROM category WHERE id = '.(int)$this->current_id_category.' LIMIT 1')) throw new Exception('An error occured while trying to get category price increment.'."\r\n\r\n".$this->mysqli->error);
			$row = $result->fetch_assoc();
			$result->free();		
			
			if ($row['price_increment']) $price_increment = $row['price_increment'];	
			...
			do{	
				...
				if ($max_price >= $min_price && $max_price <= $counter_price) break;
	...
	public function get_total_products_in_category($id_category)
	{
		...
		if (sizeof($this->ids)) {
			...
			if (!$result = $this->mysqli->query('SELECT COUNT(product_category.id_product) AS total FROM product_category WHERE id_category = "'.$id_category.'" AND id_product IN ('.implode(',',$this->ids).')')) throw new Exception('An error occured while trying to get product count.');
			$row = $result->fetch_assoc();
			$result->free();		
		}
