...
$("#page_title").html("").append("'.Yii::t('views/taxgroups/index','LABEL_TITLE').'");
...
tabs.list.toolbar.obj.attachEvent("onClick", function(id){
	var obj = this;
	var title = "'.Yii::t('global','LABEL_TAX_GROUP').'";
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/taxgroups/index','LABEL_ALERT_DELETE').'")) {
					...
					if (!opened) {					
						...
						alert("'.Yii::t('views/taxgroups/index','LABEL_ALERT_DELETE_CLOSE').'");	
					}
				}
			} else {
				alert("'.Yii::t('views/taxgroups/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').'",null,["text-align:center;"]);
tabs.list.grid.obj.attachHeader(",#text_filter_custom");
...
tabs.list.grid.obj.setColAlign("center,left");
tabs.list.grid.obj.setColSorting("na,server");
tabs.list.grid.obj.enableResizing("false,false");
