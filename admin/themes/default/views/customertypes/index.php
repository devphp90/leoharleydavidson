...
$("#page_title").html("").append("'.Yii::t('views/customertypes/index','LABEL_TITLE').'");
$("#how-to-link a").prop("title","/index.html?gestion_des_types_de_clients.html");
...
tabs.list.toolbar.obj.attachEvent("onClick", function(id){
	var obj = this;
	var title = "'.Yii::t('views/customertypes/index','LABEL_CUSTOMERS').'";
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/customertypes/index','LABEL_ALERT_DELETE').'")) {
					...
					if (!opened) {					
						...
						alert("'.Yii::t('views/customertypes/index','LABEL_ALERT_DELETE_CLOSE').'");	
					}
				}
			} else {
				alert("'.Yii::t('views/customertypes/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,'.Yii::t('views/customertypes/index','LABEL_LIST_GRID_TYPE').','.Yii::t('global','LABEL_PERCENTAGE').','.Yii::t('views/customertypes/index','LABEL_LIST_GRID_APPLY_ON_REBATE').','.Yii::t('global','LABEL_TAXABLE').'",null,["text-align:center",,"text-align:right","text-align:center","text-align:center"]);
tabs.list.grid.obj.attachHeader("&nbsp;,#text_filter_custom,#text_filter_custom,#select_filter_custom,#select_filter_custom");
...
tabs.list.grid.obj.setInitWidths("40,*,125,125,125");
tabs.list.grid.obj.setColAlign("center,left,right,center,center");
tabs.list.grid.obj.setColSorting("na,server,server,na,na");
tabs.list.grid.obj.enableResizing("false,false,false,false,false");
...
tabs.list.grid.obj.hdr.rows[2].cells[1].getElementsByTagName("INPUT")[0].name="type";
tabs.list.grid.obj.hdr.rows[2].cells[2].getElementsByTagName("INPUT")[0].name="percent_discount";
tabs.list.grid.obj.hdr.rows[2].cells[3].getElementsByTagName("SELECT")[0].name="apply_on_rebate";
tabs.list.grid.obj.hdr.rows[2].cells[4].getElementsByTagName("SELECT")[0].name="taxable";
...
tabs.list.grid.obj.attachEvent("onCheck", function(rId,cInd,state){
	if (cInd == 3) {
		$.ajax({
			url: "'.CController::createUrl('toggle_apply_on_rebate').'",
			type: "POST",
			data: { "id":rId,"apply_on_rebate":state }
		});			
	}else if (cInd == 4) {
		$.ajax({
			url: "'.CController::createUrl('toggle_taxable').'",
			type: "POST",
			data: { "id":rId,"taxable":state }
		});		
	}
});
