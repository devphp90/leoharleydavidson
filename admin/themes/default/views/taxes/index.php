...
$("#page_title").html("").append("'.Yii::t('views/taxes/index','LABEL_TITLE').'");
...
tabs.list.toolbar.obj.attachEvent("onClick", function(id){
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/taxes/index','LABEL_ALERT_DELETE').'")) {
					...
					if (!opened) {					
						...
						alert("'.Yii::t('views/taxes/index','LABEL_ALERT_DELETE_CLOSE').'");	
					}
				}
			} else {
				alert("'.Yii::t('views/taxes/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('global','LABEL_CODE').','.Yii::t('global','LABEL_TAX_NUMBER').'",null,["text-align:center"]);
tabs.list.grid.obj.attachHeader("&nbsp;,#text_filter_custom,#text_filter_custom,#text_filter_custom");

// custom text filter input
tabs.list.grid.obj._in_header_text_filter_custom=text_filter_custom;

tabs.list.grid.obj.setInitWidths("40,*,*,*");
tabs.list.grid.obj.setColAlign("center,left,left,left");
tabs.list.grid.obj.setColSorting("na,server,server,server");
tabs.list.grid.obj.enableResizing("false,false,false,false");
...
tabs.list.grid.obj.hdr.rows[2].cells[2].getElementsByTagName("INPUT")[0].name="code";
tabs.list.grid.obj.hdr.rows[2].cells[3].getElementsByTagName("INPUT")[0].name="tax_number";
