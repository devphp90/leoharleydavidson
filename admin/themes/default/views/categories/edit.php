...
'.$containerJS.'.id_category = '.($id ? $id:0).';
...
'.$containerJS.'.layout.toolbar.load('.$containerJS.'.id_category);
...
'.$containerJS.'.layout.A.dataview.obj.load("'.CController::createUrl('xml_list_section').'?id="+'.$containerJS.'.id_category,function(){ 
...
'.$containerJS.'.layout.A.dataview.obj.attachEvent("onBeforeSelect", function (id){			
	...
	$.ajax({
		...
		type: "POST",
		data: { "id":id, "id_category":'.$containerJS.'.id_category },
		beforeSend: function(){
			'.$containerJS.'.layout.A.dataview.ajaxRequests++;
		},
		success: function(data){
