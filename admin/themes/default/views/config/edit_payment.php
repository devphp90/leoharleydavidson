...
edit.layout.og_form = [];
edit.layout.obj = new dhtmlXLayoutObject("payment_layout", "1C");
...
$.ajax({
	url: "'.CController::createUrl('edit_payment_options').'",
	type: "POST",
	beforeSend: function(){
...
edit.layout.A.toolbar.obj.attachEvent("onClick",function(id){
	...
	switch (id) {
		case "save":	
			$.ajax({
				url: "'.CController::createUrl('save_payment').'",
				type: "POST",
				data: $("#payment_layout").serialize(),
				dataType: "json",
				beforeSend: function(){			
					// clear all errors					
					$("#payment_layout span.error").html("");
					$("#payment_layout *").removeClass("error");		
...
?>
<form id="payment_layout" style="width:100%; height:100%; padding:0; margin:0;"></form>