...
edit.layout.A.toolbar.load = function(){
	...
	obj.addButton("add",null,"'.Yii::t('views/config/edit_banner','BTN_ADD_BANNER').'","toolbar/add.gif","toolbar/add_dis.gif"); 
	...
	obj.attachEvent("onClick",function(id){
		switch (id) {
			...
			case "delete":
				...
				if (checked) {											
					if (confirm("'.Yii::t('views/config/edit_banner','LABEL_ALERT_DELETE').'")) {
						...
						$.ajax({
							url: "'.CController::createUrl('delete_banner').'",
							type: "POST",
							data: ids.join("&"),
							beforeSend: function(){		
								obj.disableItem(id);
					...
					alert("'.Yii::t('views/config/edit_banner','LABEL_ALERT_NO_CHECKED').'");		
				}
				break;
		}
	});	
...
edit.layout.A.toolbar.add = function(current_id){
	...
	wins_options.obj = dhxWins.createWindow("modifyBannerWindow", 0, 0, 920, 460);
		...
		obj.attachEvent("onClick",function(id){
			switch (id) {
				...
				case "save_close":
					...
					break;
				case "delete":
					if (confirm("'.Yii::t('global','LABEL_ALERT_DELETE').'")) {
						$.ajax({
							url: "'.CController::createUrl('delete_banner').'",
							type: "POST",
							data: { "ids[]":current_id },
	...
	wins_options.layout.A.tabs.obj.loadXML("'.CController::createUrl('xml_list_banner_description').'?id="+current_id);

	$.ajax({
		url: "'.CController::createUrl('edit_banner_options').'?id="+current_id,
		type: "POST",
		success: function(data){
	...
	wins_options.toolbar.save = function(id,close){			
		...
		$.ajax({
			url: "'.CController::createUrl('save_banner').'",
			type: "POST",
				...
				if (data) {
					...
					if (data.errors) {
						...
						if(!close){
							...
							wins_options.layout.A.tabs.obj.loadXML("'.CController::createUrl('xml_list_banner_description').'?id="+data.id);
...
edit.layout.A.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('global','LABEL_ENABLED').'",null,["text-align:center",,"text-align:center"]);
	
// custom text filter input
edit.layout.A.grid.obj._in_header_text_filter_custom=text_filter_custom;

edit.layout.A.grid.obj.setInitWidthsP("4,85,11");
edit.layout.A.grid.obj.setColAlign("center,left,center");
edit.layout.A.grid.obj.setColSorting("na,na,na");
edit.layout.A.grid.obj.enableResizing("false,true,false");
...
edit.layout.A.grid.obj.xmlOrigFileUrl = "'.CController::createUrl('xml_list_banner').'";
