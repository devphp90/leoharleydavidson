...
'.$containerJS.'.id_customer = '.($id ? $id:0).';
...
'.$containerJS.'.layout.toolbar.load = function(current_id){
	...
	obj.attachEvent("onClick",function(id){
		switch (id) {
			case "delete":
				if (confirm("'.Yii::t('views/customers/edit','LABEL_ALERT_DELETE').'")) {
					$.ajax({
...
'.$containerJS.'.layout.toolbar.load('.$containerJS.'.id_customer);
...
'.$containerJS.'.layout.A.dataview.obj.load("'.CController::createUrl('xml_list_section').'?id="+'.$containerJS.'.id_customer,function(){
	'.$containerJS.'.layout.A.dataview.obj.select('.$containerJS.'.layout.A.dataview.obj.idByIndex(0));
	/*if ('.$containerJS.'.id_customer) { '.$containerJS.'.layout.A.dataview.obj.select('.$containerJS.'.layout.A.dataview.obj.first()); }
	else { '.$containerJS.'.layout.A.dataview.obj.select('.$containerJS.'.layout.A.dataview.obj.idByIndex(1)); }*/
}, "xml");
...
'.$containerJS.'.layout.A.dataview.obj.attachEvent("onBeforeSelect", function (id){	
	...
	$.ajax({
		...
		type: "POST",
		data: { "id":id, "id_customer":'.$containerJS.'.id_customer },
		beforeSend: function(){
			'.$containerJS.'.layout.A.dataview.ajaxRequests++;
		},
		success: function(data){
