...
$("#page_title").html("").append("'.Yii::t('views/taxrules/index','LABEL_TITLE').'");
$("#how-to-link a").prop("title","/index.html?gestion_des_taxes.html");
...
tabs.list.toolbar.obj.attachEvent("onClick", function(id){
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/taxrules/index','LABEL_ALERT_DELETE').'")) {
					...
					if (!opened) {					
						...
						alert("'.Yii::t('views/taxrules/index','LABEL_ALERT_DELETE_CLOSE').'");	
					}
				}
			} else {
				alert("'.Yii::t('views/taxrules/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('global','LABEL_COUNTRY').','.Yii::t('global','LABEL_STATE_PROVINCE').','.Yii::t('views/taxrules/edit_info_options','LABEL_ZIP_FROM').','.Yii::t('views/taxrules/edit_info_options','LABEL_ZIP_TO').','.Yii::t('global','LABEL_ENABLED').'",null,["text-align:center",,,,,,"text-align:center"]);
tabs.list.grid.obj.attachHeader("&nbsp;,#text_filter_custom,#text_filter_custom,#text_filter_custom,#text_filter_custom,#text_filter_custom,#select_filter_custom_enableddisabled");

// custom text filter input
tabs.list.grid.obj._in_header_text_filter_custom=text_filter_custom;

// custom select filter input
tabs.list.grid.obj._in_header_select_filter_custom_yesno=select_filter_custom_yesno;

tabs.list.grid.obj._in_header_select_filter_custom_enableddisabled=select_filter_custom_enableddisabled;

tabs.list.grid.obj.setInitWidths("40,*,*,*,100,100,100");
tabs.list.grid.obj.setColAlign("center,left,left,left,left,left,center");
tabs.list.grid.obj.setColSorting("na,server,server,server,server,server,na");
...
tabs.list.grid.obj.hdr.rows[2].cells[2].getElementsByTagName("INPUT")[0].name="country_code";
tabs.list.grid.obj.hdr.rows[2].cells[3].getElementsByTagName("INPUT")[0].name="state_code";
tabs.list.grid.obj.hdr.rows[2].cells[4].getElementsByTagName("INPUT")[0].name="zip_from";
tabs.list.grid.obj.hdr.rows[2].cells[5].getElementsByTagName("INPUT")[0].name="zip_to";
