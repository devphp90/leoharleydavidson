...
'.$containerJS.'.id_tax_rule = '.($model->id ? $model->id:0).';
...
'.$containerJS.'.layout.toolbar.load = function(current_id){
	...
	obj.attachEvent("onClick",function(id){
		switch (id) {
			case "delete":
				if (confirm("'.Yii::t('views/taxrules/edit','LABEL_ALERT_DELETE').'")) {
					$.ajax({
...
'.$containerJS.'.layout.toolbar.load('.$containerJS.'.id_tax_rule);
...
'.$containerJS.'.layout.A.dataview.reloadData = function(){
	var obj = '.$containerJS.'.layout.A.dataview.obj;
	var itemCount = obj.dataCount();
	
	for (var i=1; i<itemCount; i++) {			
		obj.get(obj.idByIndex(i)).disabled = '.$containerJS.'.id_tax_rule ? 0 : 1;
	}
...
'.$containerJS.'.layout.A.dataview.obj.load("'.CController::createUrl('xml_list_section').'?id="+'.$containerJS.'.id_tax_rule,function(){ 
...
'.$containerJS.'.layout.A.dataview.obj.attachEvent("onBeforeSelect", function (id){	
	...
	$.ajax({
		...
		type: "POST",
		data: { "id":id, "id_tax_rule":'.$containerJS.'.id_tax_rule },
		beforeSend: function(){
			'.$containerJS.'.layout.A.dataview.ajaxRequests++;
		},
		success: function(data){
