...
'.$containerObj.'.layout.toolbar.load = function(current_id){
	...
	'.$containerObj.'.layout.toolbar.obj.addButton("preview",null,"'.Yii::t('global','LABEL_PREVIEW').'","toolbar/preview.png","toolbar/preview_dis.png"); 
	
	if (!'.$containerJS.'.id_subscription_contest) '.$containerObj.'.layout.toolbar.obj.disableItem("preview");
	...
	'.$containerObj.'.layout.toolbar.obj.attachEvent("onClick",function(id){
		...
		switch (id) {
			case "save":	
					...
					success: function(data){						
						...
						if (data) {
							if (data.errors) {
								...
								if (!'.$containerJS.'.id_subscription_contest) {								
									'.$containerJS.'.id_subscription_contest=data.id;									
									'.$containerObj.'.layout.toolbar.load('.$containerJS.'.id_subscription_contest);
									...
									'.$containerObj.'.layout.toolbar.obj.enableItem("preview");								
								}
				...
				break;
			case "preview":
				//window.open("'.CController::createUrl('preview').'/"+'.$containerJS.'.id_subscription_contest);
				window.open("'.CController::createUrl('preview').'?id="+'.$containerJS.'.id_subscription_contest+"&language="+'.$containerObj.'.layout.A.tabbar.obj.getActiveTab());
				break;
		}
	});	
};
