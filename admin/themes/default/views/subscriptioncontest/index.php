...
$("#page_title").html("").append("'.Yii::t('views/subscriptioncontest/index','LABEL_TITLE').'");
$("#how-to-link a").prop("title","/index.html?gestion_des_formulaires_dinscr.html");
...
tabs.list.toolbar.obj.attachEvent("onClick", function(id){
	var obj = this;
	var title = "'.Yii::t('views/subscriptioncontest/index','LABEL_TAB_LIST').'";
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/subscriptioncontest/index','LABEL_ALERT_DELETE').'")) {
					...
					if (!opened) {					
						...
						alert("'.Yii::t('views/subscriptioncontest/index','LABEL_ALERT_DELETE_CLOSE').'");	
					}
				}
			} else {
				alert("'.Yii::t('views/subscriptioncontest/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,Type,'.Yii::t('global','LABEL_NAME').','.Yii::t('global','LABEL_START_DATE').','.Yii::t('global','LABEL_END_DATE').','.Yii::t('views/subscriptioncontest/index','LABEL_GRID_TOTAL_SUBSCRIPTION').','.Yii::t('global','LABEL_ENABLED').'",null,["text-align:center",,,,,,,"text-align:right","text-align:center"]);
tabs.list.grid.obj.attachHeader("&nbsp;,#select_filter_custom_coupon,#text_filter_custom,#datetime_filter_custom,#datetime_filter_custom,&nbsp;,#select_filter_custom_enableddisabled");
...
tabs.list.grid.obj._in_header_select_filter_custom_coupon=select_filter_custom_contest;

tabs.list.grid.obj.setInitWidthsP("5,10,50,10,10,10,5");
tabs.list.grid.obj.setColAlign("center,left,left,left,left,center,center");
tabs.list.grid.obj.enableResizing("false,false,false,false,false,false,false");
tabs.list.grid.obj.setColSorting("na,server,server,server,server,server,server");
...
tabs.list.grid.obj.hdr.rows[2].cells[2].getElementsByTagName("INPUT")[0].name="name";
tabs.list.grid.obj.hdr.rows[2].cells[3].getElementsByTagName("INPUT")[0].name="start_date_start";
tabs.list.grid.obj.hdr.rows[2].cells[3].getElementsByTagName("INPUT")[1].name="start_date_end";
tabs.list.grid.obj.hdr.rows[2].cells[4].getElementsByTagName("INPUT")[0].name="end_date_start";
tabs.list.grid.obj.hdr.rows[2].cells[4].getElementsByTagName("INPUT")[1].name="end_date_end";
tabs.list.grid.obj.hdr.rows[2].cells[6].getElementsByTagName("SELECT")[0].name="active";
...
tabs.list.grid.obj.attachEvent("onRowDblClicked",function(rId,cInd){
	tabs.load_tab(rId,this.cellById(rId,2).getValue());
});
