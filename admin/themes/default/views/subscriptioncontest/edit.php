...
'.$containerJS.'.id_subscription_contest = '.($model->id ? $model->id:0).';
...
'.$containerJS.'.layout.toolbar.load = function(current_id){
	...
	obj.attachEvent("onClick",function(id){
		switch (id) {
			case "delete":
				if (confirm("'.Yii::t('views/subscriptioncontest/edit','LABEL_ALERT_DELETE').'")) {
					$.ajax({
...
'.$containerJS.'.layout.toolbar.load('.$containerJS.'.id_subscription_contest);
...
'.$containerJS.'.layout.A.dataview.obj.load("'.CController::createUrl('xml_list_section').'?id="+'.$containerJS.'.id_subscription_contest,function(){ 
...
'.$containerJS.'.layout.A.dataview.obj.attachEvent("onBeforeSelect", function (id){	
	...
	$.ajax({
		...
		type: "POST",
		data: { "id":id, "id_subscription_contest":'.$containerJS.'.id_subscription_contest },
		beforeSend: function(){
			'.$containerJS.'.layout.A.dataview.ajaxRequests++;
		},
		success: function(data){
