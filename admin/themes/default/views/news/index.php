...
$("#page_title").html("").append("'.Yii::t('views/news/index','LABEL_TITLE').'");
$("#how-to-link a").prop("title","/index.html?gestion_des_nouvelles.html");
...
tabs.list.toolbar.obj.attachEvent("onClick", function(id){
	var obj = this;
	var title = "'.Yii::t('views/news/index','LABEL_TAB_LIST').'";
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/news/index','LABEL_ALERT_DELETE').'")) {
					...
					if (!opened) {					
						...
						alert("'.Yii::t('views/news/index','LABEL_ALERT_DELETE_CLOSE').'");	
					}
				}
			} else {
				alert("'.Yii::t('views/news/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('global','LABEL_DATE').','.Yii::t('views/news/index','LABEL_IMAGE').','.Yii::t('global','LABEL_ENABLED').'",null,["text-align:center",,,,"text-align:center"]);


tabs.list.grid.obj.setInitWidthsP("5,35,20,25,15");
tabs.list.grid.obj.setColAlign("center,left,left,left,center");
tabs.list.grid.obj.enableResizing("false,false,false,false,false");
tabs.list.grid.obj.setColSorting("na,server,server,server,server");
