...
'.$containerJS.'.id_rebate_coupon = '.($model->id ? $model->id:0).';
'.$containerJS.'.type = '.$model->type.';
...
'.$containerJS.'.layout.toolbar.load = function(current_id){
	...
	obj.attachEvent("onClick",function(id){
		switch (id) {
			case "delete":
				if (confirm("'.Yii::t('views/rebatecoupon/edit','LABEL_ALERT_DELETE').'")) {
					$.ajax({
						...
						success: function(data){					
							if (window["'.$containerJS.'_obj"] && typeof(window["'.$containerJS.'_obj"].load_og_form) == "function") window["'.$containerJS.'_obj"].load_og_form();
							
							tabs.close_tab("'.$container.'", true);
...
'.$containerJS.'.layout.toolbar.load('.$containerJS.'.id_rebate_coupon);
...
'.$containerJS.'.layout.A.dataview.enableItems = function(i,id){
	...
	for (var x=0; x<itemCount; x++) {
		...
		if (((current_id == "edit_products" || current_id == "edit_categories") && ('.$containerJS.'.type == 1 || '.$containerJS.'.type == 4 || '.$containerJS.'.type == 5)) || (current_id == "edit_categories" && '.$containerJS.'.type == 3)) {
...
'.$containerJS.'.layout.A.dataview.obj.load("'.CController::createUrl('xml_list_section').'?id="+'.$containerJS.'.id_rebate_coupon,function(){ 
...
'.$containerJS.'.layout.A.dataview.obj.attachEvent("onBeforeSelect", function (id){	
	...
	$.ajax({
		...
		type: "POST",
		data: { "id":id, "id_rebate_coupon":'.$containerJS.'.id_rebate_coupon },
		beforeSend: function(){
			'.$containerJS.'.layout.A.dataview.ajaxRequests++;
		},
		success: function(data){
