...
$("#page_title").html("").append("'.Yii::t('views/users/index','LABEL_TITLE').'");
$("#how-to-link a").prop("title","/index.html?gestion_des_utilisateurs.html");
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				...
				// check if linked to other stores
				$.ajax({
					url: "'.CController::createUrl('is_user_linked_store').'",
					type: "POST",
					data: ids.join("&"),
					beforeSend: function(){		
						obj.disableItem(id);
					...
					success: function(data){	
						var delete_linked = 0;
						if (data=="true" && confirm("'.Yii::t('views/users/index','LABEL_ALERT_DELETE_LINKED').'")) delete_linked = 1;						
					
						if (confirm("'.Yii::t('views/users/index','LABEL_ALERT_DELETE').'")) {
							if (!opened) {					
								$.ajax({
									url: "'.CController::createUrl('delete').'",
									type: "POST",
									data: ids.join("&")+"&delete_linked="+delete_linked,
									complete: function(){
										obj.enableItem(id);	
									},							
									success: function(data){													
								...
								alert("'.Yii::t('views/users/index','LABEL_ALERT_DELETE_CLOSE').'");	
							}
						}
					}
				});
			} else {
				alert("'.Yii::t('views/users/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('views/users/index','LABEL_PHONE').','.Yii::t('views/users/index','LABEL_ADDRESS').','.Yii::t('views/users/index','LABEL_EMAIL').','.Yii::t('global','LABEL_ENABLED').'",null,["text-align:center",,,,,"text-align:center"]);
tabs.list.grid.obj.attachHeader("&nbsp;,#text_filter_custom,,,,#select_filter_custom_enableddisabled");
...
tabs.list.grid.obj.setInitWidthsP("5,45,15,15,15,5");
tabs.list.grid.obj.setColAlign("center,left,left,left,left,center");
tabs.list.grid.obj.setColSorting("na,server,na,na,na,na");
tabs.list.grid.obj.enableResizing("false,false,false,false,false,false");
...
tabs.list.grid.obj.hdr.rows[2].cells[1].getElementsByTagName("INPUT")[0].name="lastname";
tabs.list.grid.obj.hdr.rows[2].cells[5].getElementsByTagName("SELECT")[0].name="active";
