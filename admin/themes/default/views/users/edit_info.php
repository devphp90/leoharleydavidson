					...
					success: function(data){						
						...
						if (data) {
							if (data.errors) {
								...
								var id_tag_container = "'.$containerObj.'_lastname";
								var id_tag_selector = "#"+id_tag_container.replace(/\[/gi,"\\\\[").replace(/\]/gi,"\\\\]");
								
								var id_tag_container2 = "'.$containerObj.'_firstname";
								var id_tag_selector2 = "#"+id_tag_container2.replace(/\[/gi,"\\\\[").replace(/\]/gi,"\\\\]");
								
							
								// if firstname or lastname changed, rename tab
								if (tabs.obj.getLabel("'.$container.'") != $(id_tag_selector2).val() + " " + $(id_tag_selector).val()) {
									tabs.obj.setLabel("'.$container.'",$(id_tag_selector2).val() + " " + $(id_tag_selector).val());	
								}
													
								load_grid(tabs.list.grid.obj);		
								
								if (!'.$containerJS.'.id_user) {								
									'.$containerJS.'.id_user=data.id;
									'.$containerJS.'.layout.toolbar.load(data.id);									
									
									// when we create a new user and save, store its id and container value in array of opened tabs
