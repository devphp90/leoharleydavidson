...
<?php echo CHtml::beginForm('','post',array(
	'id'=>$containerForm,
	'style'=>'width:100%; height:100%; padding:0; margin:0; overflow:auto;',
)); ?>
<div style="padding:10px;">	
    <div class="row">
        <div style="float:left;">
            <strong><?php echo Yii::t('views/products/edit_images_crop','LABEL_ORIGINAL');?></strong>
            <div style="border: 1px solid #303030; margin-bottom:5px;">
            <?php echo CHtml::image(CController::createUrl('edit_images_crop_load_image',array('id_product'=>$product_image->id_product,'id_product_image'=>$product_image->id,'rotate'=>$rotate,'t'=>time())),Yii::t('products','LABEL_ORIGINAL_IMAGE'),array('id'=>$container.'_jcrop_target','border'=>0, 'width'=>$original_width, 'height'=>$original_height)); ?>
            </div>
            <?php echo Yii::t('views/products/edit_images_crop','LABEL_REGION_CROP');?>
        </div>
        <div style="float:left; margin-left:5px;">
            <strong><?php echo Yii::t('views/products/edit_images_crop','LABEL_PREVIEW');?></strong>
            <div style="width:<?php echo $preview_width; ?>px;height:<?php echo $preview_height; ?>px;overflow:hidden; border: 1px solid #303030;">
            <?php echo CHtml::image(CController::createUrl('edit_images_crop_load_image',array('id_product'=>$product_image->id_product,'id_product_image'=>$product_image->id,'rotate'=>$rotate,'t'=>time())),Yii::t('products','LABEL_PREVIEW_IMAGE'),array('id'=>$container.'_jcrop_preview','border'=>0, 'width'=>$preview_width, 'height'=>$preview_height)); ?>
            </div>        
        </div>
        <div style="clear:both;"></div>
    </div>    
</div>
<?php echo CHtml::hiddenField('id',$product_image->id,array('id'=>$container.'_id')); ?>
