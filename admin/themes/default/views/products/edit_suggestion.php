...
'.$containerObj.'.layout.A.toolbar.obj.addButton("add",null,"'.Yii::t('views/products/edit_suggestion','LABEL_BTN_ADD_PRODUCT').'","toolbar/add.gif","toolbar/add_dis.gif");
...
'.$containerObj.'.layout.A.toolbar.obj.attachEvent("onClick", function(id){	
	var obj = this;
	var title = "'.Yii::t('views/products/edit_suggestion','LABEL_BTN_SUGGESTED_UNDER').' "+tabs.obj.getLabel("'.$container.'");
		...
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/products/edit_suggestion','LABEL_ALERT_DELETE_PRODUCTS').'")) {
					...
					$.ajax({
						url: "'.CController::createUrl('delete_suggestion').'",
						type: "POST",
						data: ids.join("&"),
						beforeSend: function(){		
							obj.disableItem(id);
				...
				alert("'.Yii::t('views/products/edit_suggestion','LABEL_ALERT_NO_SUGGESTED_CHECKED_PRODUCT').'");	
			}
			break;
		case "export_pdf":
...
'.$containerObj.'.layout.A.toolbar.add = function(id_product){
	name = "'.Yii::t('views/products/edit_suggestion','LABEL_ADD_SUGGESTED_PRODUCTS').'";
	

	'.$containerObj.'.wins_list.obj = '.$containerObj.'.dhxWins.createWindow("addSuggestedWindow", 10, 10, 700, 380);
	...
	'.$containerObj.'.wins_list.layout.A.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('global','LABEL_SKU').','.Yii::t('global','LABEL_PRODUCT_TYPE').','.Yii::t('global','LABEL_PRICE').'",null,["text-align:center;",,,"text-align:center;","text-align:center;","text-align:right"]);
	'.$containerObj.'.wins_list.layout.A.grid.obj.attachHeader(",#text_filter_custom,#text_filter_custom,#select_filter_custom_product_type,,");
	
	// custom text filter input
	'.$containerObj.'.wins_list.layout.A.grid.obj._in_header_text_filter_custom=text_filter_custom;
	'.$containerObj.'.wins_list.layout.A.grid.obj._in_header_select_filter_custom_product_type=select_filter_custom_product_type;
	
	
	'.$containerObj.'.wins_list.layout.A.grid.obj.setInitWidthsP("5,40,26,15,14");
	'.$containerObj.'.wins_list.layout.A.grid.obj.setColAlign("center,left,left,left,right");
	'.$containerObj.'.wins_list.layout.A.grid.obj.setColSorting("na,na,na,na,na");
	'.$containerObj.'.wins_list.layout.A.grid.obj.enableResizing("false,true,true,true,true");
	...
	'.$containerObj.'.wins_list.layout.A.grid.obj.hdr.rows[2].cells[3].getElementsByTagName("SELECT")[0].name="product_type";
	
	// we create a variable to store the default url used to get our grid data, so we can reuse it later
	'.$containerObj.'.wins_list.layout.A.grid.obj.xmlOrigFileUrl = "'.CController::createUrl('xml_list_suggestion_add').'?id_product="+id_product;
	...
	'.$containerObj.'.wins_list.toolbar.save = function(id, close){
		...
		if (checked) {											
			...
			$.ajax({
				url: "'.CController::createUrl('add_suggestion').'",
				type: "POST",
...
'.$containerObj.'.layout.A.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('global','LABEL_SKU').','.Yii::t('global','LABEL_PRODUCT_TYPE').','.Yii::t('global','LABEL_PRICE').'",null,["text-align:center;",,,,"text-align:right"]);

'.$containerObj.'.layout.A.grid.obj.setInitWidthsP("4,45,25,13,13");
'.$containerObj.'.layout.A.grid.obj.setColAlign("center,left,left,left,right");
...
'.$containerObj.'.layout.A.grid.obj.xmlOrigFileUrl = "'.CController::createUrl('xml_list_suggestion',array('id'=>$id)).'";
...
'.$containerObj.'.layout.A.grid.obj.attachEvent("onDrop", function(sId,tId,dId,sObj,tObj,sCol,tCol){
	...
	$.ajax({
		url: "'.CController::createUrl('save_suggestion_sort_order',array('id'=>$id)).'",
		type: "POST",
		data: ids.join("&")
	});	
});
