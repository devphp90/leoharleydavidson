...
<div style="padding:10px;"><span id="<?php echo $container?>_country_code_errorMsg" class="error"></span>
    ...
    <div>
        ...
        <div id="<?php echo $container?>_list_do_not_ship_states">
        <?php
...
function '.$container.'_get_province_list(current_id){
	// ajax request			
	$.ajax({
		url: "'.CController::createUrl('get_province_list_do_not_ship_region').'",
		type: "POST",
		data: { "country":current_id,"container":"'.$container.'" },
		success: function(data) {					
			$("#'.$container.'_list_do_not_ship_states").html("").append(data);	
		}
	});		
}
';
