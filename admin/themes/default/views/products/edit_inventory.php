...
'.$containerObj.'.layout.toolbar.load = function(current_id){
	...
	obj.attachEvent("onClick",function(id){
		switch (id) {
			case "save":	
				$.ajax({
					url: "'.CController::createUrl('save_inventory',array('container'=>$containerObj)).'",
					type: "POST",
...
$.ajax({
	url: "'.CController::createUrl('edit_inventory_options',array('container'=>$containerObj,'id'=>$id)).'",
	type: "POST",
	beforeSend: function(){
