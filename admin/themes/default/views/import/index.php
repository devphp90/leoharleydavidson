...
$("#page_title").html("").append("'.Yii::t('views/import/index','LABEL_TITLE').'");
$("#how-to-link a").prop("title","/index.html?importer_exporter_vos_donnees.html");
...
tabs.close_tab = function(tabObj, id, select_list) {
	if (window["Tab"+id] && typeof(window["Tab"+id].has_modifications) == "function" && window["Tab"+id].has_modifications() && !confirm("'.Yii::t('global','LABEL_CONFIRM_CONTINUE_WITHOUT_SAVING').'")) return false;	
...
tabs.list.toolbar.obj.attachEvent("onClick", function(id){
	var obj = this;
	var title = "'.Yii::t('views/pub/index','LABEL_TAB_LIST').'";
	...
	switch (id) {
		case "delete":			
			...
			if (checked) {
				if (confirm("'.Yii::t('views/pub/index','LABEL_ALERT_DELETE').'")) {
					...
					if (!opened) {					
						...
						alert("'.Yii::t('views/pub/index','LABEL_ALERT_DELETE_CLOSE').'");	
					}
				}
			} else {
				alert("'.Yii::t('views/pub/index','LABEL_ALERT_NO_CHECKED').'");	
			}
			break;
		case "export_pdf":
...
tabs.list.grid.obj.setHeader("#master_checkbox,'.Yii::t('global','LABEL_NAME').','.Yii::t('views/import/edit_options','LABEL_TYPE').'",null,["text-align:center",,]);
tabs.list.grid.obj.attachHeader("&nbsp;,#text_filter_custom,#select_filter_custom_type");

// custom text filter input
tabs.list.grid.obj._in_header_text_filter_custom=text_filter_custom;

// custom select filter input
tabs.list.grid.obj._in_header_select_filter_custom_type=function select_filter_custom_type(tag,index,data){
	var obj = this;

	tag.innerHTML="<select style=\"width:90%;font-size:8pt;font-family:Tahoma;\" class=\"hdr_custom_filters\"><option value=\"\"></option><option value=\"0\">'.Yii::t('views/import/edit_options','LABEL_TYPE_0').'</option></select>";	
...
tabs.list.grid.obj.hdr.rows[2].cells[2].getElementsByTagName("SELECT")[0].name="type";
...
tabs.list.grid.obj.attachEvent("onRowDblClicked",function(rId,cInd){
	tabs.load_tab(rId,this.cellById(rId,1).getValue());
});
...
var check_import_status;
var ajaxCheckImportStatus;

$(function(){
	check_import_status = setTimeout("checkImportStatus()",10000);		
});

function checkImportStatus()
{
	if (ajaxCheckImportStatus) ajaxCheckImportStatus.abort();
	
	ajaxCheckImportStatus = $.ajax({
		url: "'.CController::createUrl('check_import_status').'",
		success: function(data){
			// a change occured
			if (data == "true") {				
				...
				$.each(tabs.obj._tabs, function(key, value) {
					if (key != "list" && key != "new") {
						if (window["Tab"+key] && window["Tab"+key].layout && window["Tab"+key].layout.C && window["Tab"+key].layout.C.grid && window["Tab"+key].layout.C.grid.load) {
							if (window["Tab"+key].wins.load && $("#Tab"+key+"_file_id").length) {
								var current_id = $("#Tab"+key+"_file_id").val();
								window["Tab"+key].wins.toolbar.load(current_id);
								window["Tab"+key].wins.load(current_id);
							}
							
							window["Tab"+key].layout.C.grid.load();
						}
					}
				});					
			}
