								...
								if (!$progress_data || $progress_data && $progress_data == $current_row) {	
									...
									foreach ($data as $col => $value) {						
										...
										switch ($id_import_columns) {
											...
											case 6:		
												if (!$type) {							
													$value = substr($value,0,$alias_length);																
												...
												if (!empty($value)) {	
													if ($image->load($value)) {
														...
														if (empty($ext) || !in_array($ext, $allowed_ext)) {
															...
															} else if ($width < $default_zoom_width || $height < $default_zoom_height) { 
						...
						while (($data = fgetcsv($handle,0,$columns_separated_with,$columns_enclosed_with,$columns_escaped_with))) {		
							...
							if (!$skip_first_row || $skip_first_row && $x > 0) {	
								...
								if (!$progress_data || $progress_data && $progress_data == $current_row) {	
									...
									$sku = isset($columns_keys[7]) ? $data[$columns_keys[7][0]]:'';
									$brand = isset($columns_keys[8]) ? $data[$columns_keys[8][0]]:'';
									$model = isset($columns_keys[9]) ? $data[$columns_keys[9][0]]:'';
									...
									if (isset($columns_keys[1]) || isset($columns_keys[2]) || isset($columns_keys[3]) || isset($columns_keys[4]) || isset($columns_keys[5]) || isset($columns_keys[6])) {																
										...
										if (isset($columns_keys[1])) {
											foreach ($columns_keys[1] as $col) {
												...
												$value = $data[$col];
												$product_description[$language_code]['name'] = $value;	
											}
										}
										...
										if (isset($columns_keys[2])) {
											foreach ($columns_keys[2] as $col) {
												$language_code = $columns[$col]['extra'];
												$value = $data[$col];
												$product_description[$language_code]['short_desc'] = $value;	
											}
										}	
										...
										if (isset($columns_keys[3])) {
											foreach ($columns_keys[3] as $col) {
												$language_code = $columns[$col]['extra'];
												$value = $data[$col];
												$product_description[$language_code]['description'] = $value;	
											}
										}			
										...
										if (isset($columns_keys[4])) {
											foreach ($columns_keys[4] as $col) {
												$language_code = $columns[$col]['extra'];
												$value = $data[$col];
												$product_description[$language_code]['meta_description'] = $value;	
											}
										}		
										...
										if (isset($columns_keys[5])) {
											foreach ($columns_keys[5] as $col) {
												$language_code = $columns[$col]['extra'];
												$value = $data[$col];
												$product_description[$language_code]['meta_keywords'] = $value;	
											}
										}		
										...
										if (isset($columns_keys[6])) {
											foreach ($columns_keys[6] as $col) {
												$language_code = $columns[$col]['extra'];
												$value = $data[$col];
												$product_description[$language_code]['alias'] = $value;	
											}
										}																																						
									}
									...
									if (!$id_product = $progress['id_product']) {		
										...
										if ($type) {
											...
											if (!$id_product) {
												...
												/* bind result variables */
												$stmt_get_product_variant_id_by_sku->bind_result($id_product_variant, $id_product, $current_qty);																											
										...
										if (sizeof($product_description)) {
											foreach ($product_description as $language_code => $row_description) {
												...
												//if (!isset($progress['product_description'][$language_code])) {		
													...
													$alias = !empty($row_description['alias']) ? $row_description['alias']:'';
													if(!$alias and $name and $type != 2){
														$new_alias = rewrite_alias($name);
												...
												if (!isset($progress['product_images'][$key_row_image])) {												
													...
													if ($id_product_image = $mysqli->insert_id) {
														...
														if ($image->load($row_image['file'])) {										
															...
															// save image LISTING
															if (!$image->resize($default_listing_width,$default_listing_height)) {
															} else if (!$image->save($targetPath.'listing/'.$filename)) {
															}
											
															// save image SUGGEST
															if (!$image->resize($default_suggest_width,$default_suggest_height)) {
															} else if (!$image->save($targetPath.'suggest/'.$filename)) {
															}
															
															// save image THUMB
															if (!$image->resize($default_thumb_width,$default_thumb_height)) {
...
function rewrite_alias($string) {
	$title = trim($string);
	$title = str_replace(' ','-',$title);
	$title = str_replace('\'','',$title);
	$title = strtolower($title);
	$title = str_replace(array(',','ë','à','À','â','Â','é','É','è','È','ê','Ê','î','Î','ô','Ô','û','Û',' ','&','.','_','\'','/','"',':','(',')','!','?','$'),array('-','e','a','a','a','a','e','e','e','e','e','e','i','i','o','o','u','u','-','et','-','-','','','','-','-','','','',''),$title);
