<?php
class ShipOnlyIntoThisRegionsForm extends CFormModel
{
	// database fields
	public $id=0; 
	...
	{		
		if (Tbl_ConfigShipOnlyRegion::model()->count('id!=:id AND country_code=:country_code AND state_code=:state_code',array(':id'=>$this->id,':country_code'=>$this->country_code,':state_code'=>$this->state_code))) {
		...
		}elseif (Tbl_ConfigShipOnlyRegion::model()->count('country_code=:country_code AND state_code=:state_code',array(':country_code'=>$this->country_code,':state_code'=>'')) && !empty($this->state_code)) {
	...
	public function save()
	{
		...
		// edit or new
		if ($this->id) {
			$model = Tbl_ConfigShipOnlyRegion::model()->findByPk($this->id);	
		} else {
			$model = new Tbl_ConfigShipOnlyRegion;	
