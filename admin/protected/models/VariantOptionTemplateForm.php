<?php
class VariantOptionTemplateForm extends CFormModel
{
	// database fields
	public $id=0; //product variant group option id
	public $id_tpl_product_variant_group=0;
	//public $sku;
	...
	public $old_filename;
	public $sort_order=0;	
	public $tpl_product_variant_group_option_description=array();
	...
	{			
		...
		if ($pvg = Tbl_TplProductVariantGroup::model()->findByPk($this->id_tpl_product_variant_group)) {	
			/*if (empty($this->sku)) {
				$this->addError('sku',Yii::t('global','ERROR_EMPTY'));
			} else if (Tbl_TplProductVariantGroupOption::model()->count('id!=:id AND id_tpl_product_variant_group=:id_tpl_product_variant_group AND sku=:sku',array(':id'=>$this->id,':id_tpl_product_variant_group'=>$this->id_tpl_product_variant_group,':sku'=>$this->sku))) {
			...
			foreach (Tbl_Language::model()->active()->findAll() as $value) {		
				...
				// name is required
				if (empty($this->tpl_product_variant_group_option_description[$value->code]['name'])) {
					$this->addError('tpl_product_variant_group_option_description['.$value->code.'][name]',Yii::t('global','ERROR_EMPTY'));	
				} else if (isset($columns['name']) && strlen($name) > $columns['name']) {
					$this->addError('tpl_product_variant_group_option_description['.$value->code.'][name]',Yii::t('global','ERROR_MAXLENGTH').$columns['name']);
				}
			}			
	...
	public function save()
	{
		...
		// edit or new
		if ($this->id) {
			$model = Tbl_TplProductVariantGroupOption::model()->findByPk($this->id);	
		} else {
			$model = new Tbl_TplProductVariantGroupOption;	
			$model->id_user_created = $current_id_user;			
			$model->date_created = $current_datetime;					
			
			$criteria=new CDbCriteria; 
			$criteria->condition='id_tpl_product_variant_group=:id_tpl_product_variant_group'; 
			$criteria->order='sort_order DESC';				
			$criteria->params=array(':id_tpl_product_variant_group'=>$this->id_tpl_product_variant_group); 				
			$model->sort_order = (Tbl_TplProductVariantGroupOption::model()->find($criteria)->sort_order)+1;			
			
		}		
		
		$model->id_tpl_product_variant_group = $this->id_tpl_product_variant_group;
		//$model->sku = $this->sku;	
		...
		if ($model->save()) {		
			...
			foreach ($this->tpl_product_variant_group_option_description as $code => $value) {
				$criteria=new CDbCriteria; 
				$criteria->condition='id_tpl_product_variant_group_option=:id_tpl_product_variant_group_option AND language_code=:language_code'; 
				$criteria->params=array(':id_tpl_product_variant_group_option'=>$model->id,':language_code'=>$code); 					
				
				if (!$model_description = Tbl_TplProductVariantGroupOptionDescription::model()->find($criteria)) {
					$model_description = new Tbl_TplProductVariantGroupOptionDescription;				
					$model_description->id_tpl_product_variant_group_option = $model->id;
