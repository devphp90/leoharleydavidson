<?php
class OptionsShipOnlyIntoThisRegionsForm extends CFormModel
{
	// database fields
	public $id=0; 
	...
	{		
		if (Tbl_OptionsShipOnlyRegion::model()->count('id!=:id AND id_options=:id_options AND country_code=:country_code AND state_code=:state_code',array(':id'=>$this->id,':id_options'=>$this->id_options,':country_code'=>$this->country_code,':state_code'=>$this->state_code))) {
			$this->addError('country_code',Yii::t('global','ERROR_EXIST'));
			$this->addError('state_code',Yii::t('global','ERROR_EXIST'));
		}elseif (Tbl_OptionsDoNotShipRegion::model()->count('id_options=:id_options AND country_code=:country_code AND state_code=:state_code',array(':id_options'=>$this->id_options,':country_code'=>$this->country_code,':state_code'=>$this->state_code))) {
		...
		}elseif (Tbl_OptionsShipOnlyRegion::model()->count('country_code=:country_code AND state_code=:state_code',array(':country_code'=>$this->country_code,':state_code'=>'')) && !empty($this->state_code)) {
	...
	public function save()
	{
		...
		// edit or new
		if ($this->id) {
			$model = Tbl_OptionsShipOnlyRegion::model()->findByPk($this->id);	
		} else {
			$model = new Tbl_OptionsShipOnlyRegion;	
