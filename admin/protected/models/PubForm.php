...
class PubForm extends CFormModel
{
	...
	public $name;
	public $width=0;
	public $display_in_column=0;
	public $display_in_page=0;
	public $display_start_date;
	public $display_end_date;
	public $pub_description=array();
	...
	{	/*		
		...
		foreach (Tbl_Language::model()->active()->findAll() as $value) {							
			// file is required
			/*
			$image = $this->pub_description[$value->code]['filename'];
			if (empty($filename)) {
				$this->addError('pub_description['.$value->code.'][filename]',Yii::t('global','ERROR_EMPTY'));	
			} 
		}*/
		...
		foreach ($this->pub_description as $code => $value) {
			switch ($value['url_type']) {
				...
				// url
				case 1:	
					if (empty($value['url'])) $this->addError('pub_description['.$code.'][url]',Yii::t('global','ERROR_EMPTY'));
					break;
				// cmspage
				case 2:
					if (empty($value['id_cmspage'])) $this->addError('pub_description['.$code.'][id_cmspage]',Yii::t('global','ERROR_EMPTY'));
					break;
				// registration contest
				case 3:
					if (empty($value['id_subscription_contest'])) $this->addError('pub_description['.$code.'][id_subscription_contest]',Yii::t('global','ERROR_EMPTY'));
					break;
			}
		}
	...
	public function save()
	{
		...
		$image_base_path = $app->params['root_url'].'_images/pub/';
		
		// edit or new
		if ($this->id) {
			$model = Tbl_Pub::model()->findByPk($this->id);	
		} else {
			$model = new Tbl_Pub;	
		...
		$model->display_in_column = $this->display_in_column;
		$model->display_in_page = $this->display_in_page;
		...
		if ($model->save()) {		
			...
			foreach ($this->pub_description as $code => $value) {
				...
				$criteria->condition='id_pub=:id_pub AND language_code=:language_code'; 
				$criteria->params=array(':id_pub'=>$model->id,':language_code'=>$code); 					
				
				if (!$model_description = Tbl_PubDescription::model()->find($criteria)) {
					$model_description = new Tbl_PubDescription;				
					$model_description->id_pub = $model->id;
