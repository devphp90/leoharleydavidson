dhtmlXGridObject.prototype.toPDF=function(url,mode,header,footer,rows,title,desc){
	mode = mode || "color";	
	...
	*/
	...
	d.innerHTML = '<form id="'+uid+'" method="post" target="_blank" action="'+url+'" accept-charset="utf-8"  enctype="application/x-www-form-urlencoded"><input type="hidden" name="grid_xml" id="grid_xml"/><input type="hidden" name="grid_title" id="grid_title"/><input type="hidden" name="grid_desc" id="grid_desc"/></form>';
	document.getElementById(uid).firstChild.value = xml_top(mode).replace("\u2013", "-") + xml_body() + xml_end();
	document.getElementById(uid).grid_title.value = title;
	document.getElementById(uid).grid_desc.value = desc;
